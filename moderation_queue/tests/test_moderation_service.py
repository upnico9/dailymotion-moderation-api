...
@pytest.fixture
def service(video_repo, video_log_repo):
    """Service with a real EventDispatcher so listeners actually fire."""
    dispatcher = EventDispatcher()
    return ModerationService(video_repo, video_log_repo, dispatcher)
...
class TestAddVideo:
    ...
    def test_dispatches_event_and_creates_log(self, service, video_repo, video_log_repo):
...
class TestGetVideo:
    def test_returns_already_assigned_video(self, service, video_repo, video_log_repo):
        ...
        video_log_repo.create.assert_not_called()

    def test_assigns_next_pending_and_creates_log(self, service, video_repo, video_log_repo):
    ...
    def test_returns_none_when_queue_empty(self, service, video_repo, video_log_repo):
        video_repo.get_assigned.return_value = None
...
class TestFlagVideo:
    def test_flag_spam_success(self, service, video_repo, video_log_repo):
        video = _make_video(assigned_moderator="alice")
    ...
    def test_flag_not_spam_success(self, service, video_repo, video_log_repo):
    ...
    def test_video_not_found_raises(self, service, video_repo, video_log_repo):
    ...
    def test_not_assigned_to_moderator_raises(self, service, video_repo, video_log_repo):
        video = _make_video(assigned_moderator="bob")
    ...
    def test_unassigned_video_raises(self, service, video_repo, video_log_repo):
    ...
    def test_already_flagged_raises(self, service, video_repo, video_log_repo):
    ...
    def test_invalid_status_raises(self, service, video_repo, video_log_repo):
        video = _make_video(assigned_moderator="alice")
        video_repo.get_by_id.return_value = video
